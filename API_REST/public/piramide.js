...
// Renomeado para evitar conflito com "allOcorrencias" já declarado
// globalmente em dashboard.js (scripts não-módulo compartilham o
// mesmo escopo global; duas declarações "let" com o mesmo nome
// quebram o parsing do script inteiro).
let ocorrenciasPiramide = [];

async function loadOcorrencias() {
    try {
        const res = await fetch('/ocorrencia');
        if (!res.ok) throw new Error('HTTP ' + res.status);
        ocorrenciasPiramide = await res.json();
    } catch (err) {
...
function filterOcorrencias() {
    ...
    return ocorrenciasPiramide.filter(r => {
        if (uFilt !== 'todas' && r.unidade !== uFilt) return false;
        if (mFilt > 0) { const m = new Date(r.data_ocorrencia || '').getMonth() + 1; if (m !== mFilt) return false; }
        if (aFilt) { const a = new Date(r.data_ocorrencia || '').getFullYear().toString(); if (a !== aFilt) return false; }
        return true;
    });
}
...
function filterAlertasPir() {
    ...
    const mFilt = parseInt(document.getElementById('fil-pir-mes')?.value || 0);
    const aFilt = document.getElementById('fil-pir-ano')?.value || '';
    return (typeof allAlertas !== 'undefined' ? allAlertas : []).filter(r => {
        if (uFilt !== 'todas' && r.unidade !== uFilt) return false;
        if (mFilt > 0) { const m = new Date(r.data_registro || r.Data_ocorrencia || '').getMonth() + 1; if (m !== mFilt) return false; }
        if (aFilt) { const a = new Date(r.data_registro || r.Data_ocorrencia || '').getFullYear().toString(); if (a !== aFilt) return false; }
        return true;
    });
}
...
function renderTabelaMensal(ocs, als) {
    ...
    const indicadores = [
        ...
        { tipo: 'PROATIVO', nome: 'TOTAL ALERTAS',          unid: 'Quant.', melhor: '↑', filtro: () => true,                                           src: 'al' },
    ];
...
document.addEventListener('DOMContentLoaded', () => { setTimeout(() => renderPiramide(), 500); });